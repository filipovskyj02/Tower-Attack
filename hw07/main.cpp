...
class CIndex
{
  ...
  CIndex ( T_ data, C_ comparator = C_{}) : kont(data), cmp(comparator) {} 
  
  
  
  bool equals(const typename T_::value_type & a,const typename T_::value_type & b)const{
   
    return (((!cmp(a,b) and !cmp(b,a)))) ;
  }

  bool notEqual(const typename T_::value_type & a,const typename T_::value_type & b)const{
    return (!((!cmp(a,b) and !cmp(b,a)))) ;
  }
  set<long unsigned int> search(const T_ & needle)const{
    set<size_t> indexes;
    if (needle.empty()){
      for (size_t i = 0; i < kont.size();i++){
        indexes.emplace(i);
      }
    return indexes;
    }
    for (auto i = kont.begin(); i != kont.end(); i++){
      ...
      if (equals(*i,*needle.begin())){
        auto j = needle.begin();
        auto startIn = i;
        auto m = i;
        
        while (j != needle.end() and m != kont.end()){
          if (notEqual(*j,*m) ) break;
          j++;
          m++;
          
        }
        if (j == needle.end()){
          indexes.emplace((distance(kont.begin(), startIn)));
